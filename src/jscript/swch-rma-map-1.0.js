var ContentController = function (
    ...
    themeButtonSelector, chartButtonSelector) {
...
ContentController.prototype.updateCharts = function (selections, selectionName) {     
    if (this.chartManager) {
        if (this.chartView) {
            this.chartManager.fetchChartData(selections, selectionName);
        }
    }
}
...
var RMAMap = function (
    containerSelector, mapSelector, countySelector, stateSelector, themeTitleSelector, 
    ...
    this.stateOption = stateSelector;
    ...
    $(this.countyOption).click({obj: this}, function (event) {
        var map = event.data.obj;
        
        if (!$(this).hasClass("active")) {
            if (map.controller && map.controller.themeManager) {
                map.controller.themeManager.fetchMapData("county");
                $(map.stateOption).removeClass("active");
                $(this).addClass("active");
            }
        }
    });  
     
    $(this.countyOption).keydown({obj: this}, function (event) {
        if ((event.keyCode === 13 || event.keyCode === 32)) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
    ...
    $(this.stateOption).click({obj: this}, function (event) {
        var map = event.data.obj;  
        
        if (!$(this).hasClass("active")) {
            if (map.controller && map.controller.themeManager) {
                map.controller.themeManager.fetchMapData("state");
                $(map.countyOption).removeClass("active");
                $(this).addClass("active");
            }
        }
    });
       
    $(this.stateOption).keydown({obj: this}, function (event) {
        if ((event.keyCode === 13 || event.keyCode === 32)) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
...
RMAMap.prototype.loadMap = function () {
    ...
    this.themeLayer = new ol.layer.Image({
        type: "county",
    ...
    this.olMap = new ol.Map({
        ...
        layers: [this.basemap, this.themeLayer, stateLayer],
    ...
    if (!this.mapLoaded) {
        if (this.controller && this.controller.themeManager) {
            this.controller.themeManager.fetchMapData(this.activeTheme.reportingUnit);
        }
    }   
...
RMAMap.prototype.updateCharts = function () {      
    if (this.controller) {   
        ...
        this.controller.updateCharts(selections, this.currentSelectionName);
    }
}
...
RMAMap.prototype.updateTheme = function (themeName, themeData, method, classCount, reportingUnit, measure, isFloat) {
    var array = this.objectToArray(themeData);
    this.resetSearchBox();
    
    this.activeTheme.loaded = true;
    this.activeTheme.name = themeName; 
    this.activeTheme.data = themeData;
    this.activeTheme.reportingUnit = reportingUnit;
    this.activeTheme.measure = measure;
    var styleClasses;        
    
    $(this.themeTitle).html(themeName);
    
    if (reportingUnit == "state") {
        this.activeTheme.labelResolution = 4000;
        this.themeLayer.set("type", "state");
        this.themeLayer.setSource(this.stateSource);
    }
    else {
        this.activeTheme.labelResolution = 1000;
        this.themeLayer.set("type", "county");
        this.themeLayer.setSource(this.countySource);
    }

    this.activeTheme.digitCount = 0;
    if (isFloat) this.activeTheme.digitCount = this.getDigitCount(array);
    ...
    this.activeTheme.styleClasses = styleClasses;
    this.updateLegend(styleClasses, this.activeTheme.digitCount);
    this.clearSelections();
...
var ThemeManager = function (
    ...
    causeCaptionSelector, settingsCaptionSelector, parameterListSelector, metricSelector, yearSelector, 
    commoditySelector, causeSelector, allOptionSelector, classCountSelector, classMethodSelector, 
    measurementUnitSelector, clearSelector, updateSelector, cancelSelector, bookendSelector) { 
    ...
    $(this.updateButton).click({obj: this}, function (event) {
        ...
        if (manager.activeTheme) {
            manager.fetchMapData(manager.activeTheme.reportingUnit);    
            manager.closeForms();
        }
    });
...
ThemeManager.prototype.updateTheme = function (metric, startYear, endYear, commodities, causes, commodityLookup, causeLookup, unit, measure) {
    this.activeTheme = {
        metric: metric,
...
ThemeManager.prototype.fetchMapData = function (reportingUnit) {   
    ...
    if (c) {
        ...
        if (map && container.length > 0) {  
            ...
            if ((metric == "payment_acreage" || metric == "liability" || metric == "subsidy") && startYear < 2001) {
                startYear = 2001;
            }
            ...
            $.get("payment-map-data.php", params, function (data) {
                if (data != "") {
                    try {
                        ...
                        manager.updateTheme(metric, startYear, endYear, commodities, causes, commodityLookup, causeLookup, reportingUnit, measure);
                        map.updateTheme(metricLabel, manager.thematicData, method, count, reportingUnit, measure, isFloat);
                    }
                    catch (err) {
                        c.stopLoadAnimation();
...
ThemeManager.prototype.updateCaptions = function (metricLabel, timeSpan, commodities, causes, method, count, commodityLookup, causeLookup) {      
    ...
    this.themeCaptions.metric = metricLabel;
    this.themeCaptions.dates = timeSpan;
    this.themeCaptions.commodity = commodityLabel;
    this.themeCaptions.cause = causeLabel;
    this.themeCaptions.settings = settingsLabel;
    this.changeCaptions();
}

ThemeManager.prototype.changeCaptions = function () {
    $(this.metricCaption).html(this.themeCaptions.metric);
    $(this.yearCaption).html(this.themeCaptions.dates);
    $(this.commodityCaption).html(this.themeCaptions.commodity);
    $(this.causeCaption).html(this.themeCaptions.cause);
    $(this.settingsCaption).html(this.themeCaptions.settings);
}


var ChartManager = function (pageSelector, containerSelector, sectionSelector, metricSelector,
    ...
    previousYearSelector, nextYearSelector, previousMonthSelector, nextMonthSelector, seriesChartClearSelector, 
    barChartClearSelector, monthlyChartClearSelector, chartPopupSelector, chartPopupCanvasSelector, chartPopupCloseSelector) { 
    ...
    this.previousYearButton = previousYearSelector;
    this.nextYearButton = nextYearSelector;
    this.previousMonthButton = previousMonthSelector;
    this.nextMonthButton = nextMonthSelector;
    ...
    $("body").on("click", this.barChart + " " + this.bar + ".non-zero", {obj: this}, function (event) {
        ...
        if ($(this).attr("class") == barClass + " active") {    
            ...
            manager.updateSeriesChart(section, chartIndex, "bar", "", "", false);
            manager.updateMonthlyInteractiveChart(section, chartIndex, "bar", "", "", false);
        }
        else {
            ...
            manager.updateSeriesChart(section, chartIndex, "bar", code, label, false);
            manager.updateMonthlyInteractiveChart(section, chartIndex, "bar", code, label, false);
        }
    });    
    ...
    $("body").on("click", this.dot, {obj: this}, function (event) {
        ...
        if (manager.activeTheme) {  
            ...
            if ($(this).attr("class") == dotClass + " active") {  
                ...
                manager.updateBarChart(section, chartIndex, "annual", -1);
                manager.updateMonthlyInteractiveChart(section, chartIndex, "annual", "", "", false);
            }
            else {
                dots.attr("class", dotClass);
                var index = dots.index($(this));
                $(this).attr("class", dotClass + " active");
                manager.updateBarChart(section, chartIndex, "annual", index);
                manager.updateMonthlyInteractiveChart(section, chartIndex, "annual", index, "", false);
            }
        }
    }); 
   
    $(this.previousYearButton).click({obj: this}, function (event) {
        var manager = event.data.obj; 
        
        if (manager.activeTheme) {
            var section = $(this).parents(manager.section);
            manager.traverseTimeSeriesBackward(section, "annual");
        }
    });  
    
    $(this.previousYearButton).keydown({obj: this}, function (event) {     
        if (event.keyCode === 13 || event.keyCode === 32) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
    ...
    $(this.nextYearButton).click({obj: this}, function (event) {
        var manager = event.data.obj; 
        
        if (manager.activeTheme) {            
            var section = $(this).parents(manager.section);
            manager.traverseTimeSeriesForward(section, "annual");
        }
    });  
    
    $(this.nextYearButton).keydown({obj: this}, function (event) {    
        if (event.keyCode === 13 || event.keyCode === 32) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
    ...
    $("body").on("click", this.monthlyInteractiveChart + " " + this.monthlyBar, {obj: this}, function (event) {
        ...
        if (manager.activeTheme) {  
            ...
            if ($(this).attr("class") == barClass + " active") {  
                ...
                manager.updateSeriesChart(section, chartIndex, "monthly", "", "", false);
                manager.updateBarChart(section, chartIndex, "monthly", -1);
            }
            else {
                bars.attr("class", barClass);
                var index = bars.index($(this));
                $(this).attr("class", barClass + " active");
                manager.updateSeriesChart(section, chartIndex, "monthly", index, "", false);
                manager.updateBarChart(section, chartIndex, "monthly", index);
            }
        }
    }); 
   
    $(this.previousMonthButton).click({obj: this}, function (event) {
        var manager = event.data.obj; 
        
        if (manager.activeTheme) {
            var section = $(this).parents(manager.section);
            manager.traverseTimeSeriesBackward(section, "monthly");
        }
    });  
    
    $(this.previousMonthButton).keydown({obj: this}, function (event) {     
        if (event.keyCode === 13 || event.keyCode === 32) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
    ...
    $(this.nextMonthButton).click({obj: this}, function (event) {
        var manager = event.data.obj; 
        
        if (manager.activeTheme) {            
            var section = $(this).parents(manager.section);
            manager.traverseTimeSeriesForward(section, "monthly");
        }
    });  
    
    $(this.nextMonthButton).keydown({obj: this}, function (event) {    
        if (event.keyCode === 13 || event.keyCode === 32) {
            $(this).trigger("click");
            $("body").removeClass("no-outline");
        }
...
ChartManager.prototype.fetchChartData = function (selections, selectionName) {
    var c = this.controller;
    
    if (c.themeManager) {
        var activeTheme = c.themeManager.getTheme();
        var manager = this;
                ...
                $.get("payment-chart-data.php", params, function (data) {
                    if (data != "") {
                        try {
                            ...
                            $(manager.page).show();                        
                            manager.loadCharts(selectionName, dataObj);
                        }
                        catch (err) {
                            c.stopLoadAnimation();
                            alert("There was an error loading data");
...
ChartManager.prototype.loadCharts = function (selectionName, data) {
    if (this.controller.themeManager) {
        var activeTheme = this.controller.themeManager.getTheme();
...
ChartManager.prototype.updateSeriesChart = function (container, chartIndex, referrer, selection, chartLabel, initialize) {    
    if (container.length > 0 && chartIndex < this.sectionDetails.length && this.activeTheme) {   
        ...
        if (this.activeTheme.data && this.activeTheme.years && charts.length > 0) {
            ...
            if (referrer == "monthly" && data.hasOwnProperty("monthly") && selection != "" && selection > -1 && selection < 12) {
                subData = data["monthly"];
                
                if (subData.hasOwnProperty("year")) {
                    subData = subData["year"]; 
                    chartData = [];
                    
                    for (var year in subData) {
                        if (subData.hasOwnProperty(year)) {
                            if (selection < subData[year].length) {
                                chartData.push(subData[year][selection]);
                            }
                        }
                    }
                }
                ...
                chartLabel = months[selection] + " totals, " + details.chartLabel;
            }
            else if (data.hasOwnProperty("annual")) {
                subData = data["annual"];     

                if (subData.hasOwnProperty(details.groupVariable)) {
                    subData = subData[details.groupVariable];
                    
                    if (selection == "") {
                        chartData = subData["total"];
                        chartLabel = "Annual totals, " + details.chartLabel;
                    }
                    else if (subData.hasOwnProperty(selection)) {
                        chartData = subData[selection];   
                        chartLabel = "Annual " + chartLabel.toLowerCase() + " totals";
                    }
                }
            }
            ...
            if (chartData) {                 
                ...
                charts.empty();                
                this.createTimeSeriesChart(chart, chartData, this.activeTheme.years, index, chartLabel, details.color);
            }
        }
    }
}

ChartManager.prototype.updateBarChart = function (container, chartIndex, referrer, selection) {   
    if (container.length > 0 && this.activeTheme) {   
        ...
        if (this.activeTheme.data && this.activeTheme.years && charts.length > 0) {
            ...
            if (referrer == "") referrer = "annual";

            if (referrer == "annual" && selection > -1) {
                yearLabel = this.activeTheme.years[selection];
            }
            else if (this.activeTheme.startYear == this.activeTheme.endYear) {
                yearLabel = this.activeTheme.startYear;
            }
            else {
            ...
            if (referrer == "monthly") {               
                if (this.activeTheme.startYear == this.activeTheme.endYear) {
                    yearLabel = this.activeTheme.startYear;
                }
                else {
                    yearLabel = this.activeTheme.startYear + "–" + this.activeTheme.endYear;
                ...
                if (selection > -1 && selection < 12) {
                    var months = ["January","February","March","April","May","June","July","August","September","October","November","December"];
                    chartLabel = months[selection] + " " + yearLabel + " totals " + details.titleLabel;
                } 
                else {
                    chartLabel = yearLabel + " totals " + details.titleLabel;
                }
            }
            else {
                chartLabel = yearLabel + " totals " + details.titleLabel;
            ...
            if (data.hasOwnProperty(referrer) && this.activeTheme.hasOwnProperty(details.groupLookup)) {
                subData = data[referrer];
            ...
            if (chartData) {
                charts.empty();
                var chart = charts.get()[0];
                this.createBarChart(chart, chartData, nameLookup, selection, chartLabel, details.color);
            }
        }
    }
}

ChartManager.prototype.updateMonthlyInteractiveChart = function (container, chartIndex, referrer, selection, chartLabel, initialize) {   
    if (container.length > 0 && this.activeTheme) {   
        ...
        if (this.activeTheme.data && charts.length > 0) {
            ...
            if (referrer == "annual" && selection != "" && selection > -1) {
                yearLabel = this.activeTheme.years[selection];
            }
            else if (this.activeTheme.startYear == this.activeTheme.endYear) {
                yearLabel = this.activeTheme.startYear;
            ...
            if (data.hasOwnProperty("monthly")) {
                ...
                if (referrer == "annual" && subData.hasOwnProperty("year")) {
                    chartData = subData["year"];

                    if ((selection == "" || selection == -1) && chartData.hasOwnProperty("total")) {
                        chartLabel = yearLabel + " monthly totals, " + details.chartLabel;
                    }
                    else {
                        selection = this.activeTheme.startYear + selection;
                        chartLabel = selection + " monthly totals, " + details.chartLabel;
                    }
                }
                else if (subData.hasOwnProperty(details.groupVariable)) {
                    chartData = subData[details.groupVariable];
                    ...
                    if (selection == "" || selection == -1) {
                        ...
                        chartLabel = yearLabel + " monthly " + chartLabel.toLowerCase() + " totals";
                    }
                }
            }
            ...
            if (chartData) {
                ...
                var chart = charts.get()[0];              
                charts.empty();
                this.createMonthlyBarChart(chart, chartData, selection, index, chartLabel, details.color);    
            }
        }
    }
}

ChartManager.prototype.createTimeSeriesChart = function (chart, data, years, yearIndex, chartLabel, color) { 
...
ChartManager.prototype.createBarChart = function (chart, data, nameLookup, dataIndex, chartLabel, color) {
    var totals = [];
    var totalsSub = [];
    var barCount = 0;
    var barWidth = 16;
    ...
    for (var key in data) {
        if (data.hasOwnProperty(key)) {
            if (dataIndex > -1 && dataIndex < data[key].length) {            
                value = Number(data[key][dataIndex]);
            }
            else {
...
ChartManager.prototype.createMonthlyBarChart = function (chart, data, variable, index, chartLabel, color) {
    ...
    if (variable == "" || variable < 0 || !data.hasOwnProperty(variable)) {
        for (var i = 0; i < 12; i++) {
...
ChartManager.prototype.traverseTimeSeriesBackward = function (section, timeVariable) { 
    if (section.length > 0) { 
        ...
        if (chartIndex > -1 && chartIndex < this.sectionDetails.length) {
            if (timeVariable == "monthly") {
                ...
                if (bars.length > 0) {    
                    ...
                    this.updateSeriesChart(section, chartIndex, "monthly", index, "", false);
                    this.updateBarChart(section, chartIndex, "monthly", index);
                } 
            }
            else {        
                ...
                if (dots.length > 0) {    
                    ...
                    this.updateBarChart(section, chartIndex, "annual", index);
                    this.updateMonthlyInteractiveChart(section, chartIndex, "annual", index, "", false);
                } 
            }
        }
    }
...
ChartManager.prototype.traverseTimeSeriesForward = function (section, timeVariable) { 
    if (section.length > 0) {        
        var container = section.parents(this.container);
        var chartIndex = container.find(this.section).index(section);
...
tour.addStop(
    "#tour-layer-anchor",
    "Toggle between county and state layers.",
    null,
    null
);
